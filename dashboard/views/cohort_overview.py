...
def render(is_researcher: bool) -> None:
    ...
    # ------------------------------------------------------------------
    # Feature correlation heatmap (researcher only)
    # ------------------------------------------------------------------
    if is_researcher:
        st.markdown(
            "**Feature correlation heatmap** (Researcher view — raw EI/engagement scores)"
        )
    ...
    fig.add_trace(go.Scatter(
        ...
        y=weekly["avg_logins"],
        mode="lines+markers",
        name="Avg. weekly logins",
    ))
    fig.add_trace(go.Scatter(
        x=weekly["week_number"],
        y=weekly["avg_forum_posts"],
        mode="lines+markers",
        name="Avg. forum posts",
    ))
    fig.add_trace(go.Scatter(
        x=weekly["week_number"],
        y=weekly["avg_video_completion"] * 10,
        mode="lines+markers",
        name="Avg. video completion (×10 for scale)",
    ))
    fig.update_layout(
    ...
    # ------------------------------------------------------------------
    # Researcher-only: ETL quality log + model comparison table
    # ------------------------------------------------------------------
    if is_researcher:
        st.divider()
